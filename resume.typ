...
#cventry(
  ...
  - 项目目标：利用 Google Cloud Platform (GCP) 的多种云服务，构建一个高效的电影推荐系统。该项目的核心目标是无缝集成至少三种不同的 GCP 服务，展示我们在创新和解决现实世界挑战方面的能力。
  - 项目简介：
...
#cventry(
  ...
  - 项目目标：通过模拟修改后的（简化版）PowerPC 604和620架构，评估不同架构参数对CPU设计的影响。项目假设32位架构执行RISC-V指令集的子集，目标是探索和分析不同架构参数对性能的影响。
  - 项目简介：
    - 该项目模拟了一种简化版的PowerPC架构，支持以下10条RISC-V指令：fld、fsd、add、addi、slt、fadd、fsub、fmul、fdiv、bne。模拟器设计旨在读取输入文件中的RISC-V汇编程序，识别并执行这些指令。通过参数化设计，允许实验和调整不同的架构参数，如NF（功能单元数量）、NI（指令数目）、NW（写回寄存器数量）、NR（重命名寄存器数量）和NB（分支预测缓冲区大小），以评估这些参数对CPU性能的影响。
  - 架构设计：
  ...
  - 项目成就：
    - 成功开发了一个可调节参数的CPU模拟器，能够灵活评估不同架构配置对性能的影响。提供了详细的性能数据，为后续的CPU设计和优化提供了依据。
]
...
#cventry(
  ...
  - 用户界面设计：设计简单直观的界面，方便用户输入电影信息并获取推荐结果。
  
  - 项目成就：成功开发了一个结合机器学习和深度学习的混合推荐系统，实现了高效的电影分类和个性化推荐。系统能够根据用户偏好，智能地推荐符合个人口味的电影，显著提升用户的观影体验。
]
