...
- 项目目标：通过模拟简化版 PowerPC 604 和 620 架构，评估不同架构参数对 CPU 设计的影响。项目假设 32 位架构执行 RISC-V 指令集的子集，目标是探索和分析不同架构参数对性能的影响。
- 项目简介：
  - 该项目模拟了一种简化版的PowerPC架构，支持以下 10 条 RISC-V 指令：fld、fsd、add、addi、slt、fadd、fsub、fmul、fdiv、bne。模拟器设计旨在读取输入文件中的 RISC-V 汇编程序，识别并执行这些指令。通过参数化设计，允许实验和调整不同的架构参数，如 NF（功能单元数量）、NI（指令数目）、NW（写回寄存器数量）、NR（重命名寄存器数量）和 NB（分支预测缓冲区大小），以评估这些参数对 CPU 性能的影响。
- 架构设计：
  - 指令集实现：模拟器支持一组核心 RISC-V 指令，能够有效执行与这些指令相关的计算和操作。
  - 架构参数化：支持对 NF、NI、NW、NR 和 NB 等架构参数的调整，以便分析不同参数设置对执行周期、暂停次数和总线利用率的影响。
  - 性能分析：记录模拟过程中关键性能指标，包括执行周期、计算暂停次数及 CDB 总线的利用情况，以便后续的性能优化和分析。
]
...
- 技术方法：
  ...
  - 深度学习模型：使用 RNN、BERT、LSTM 等模型，进一步提升推荐系统的准确性和智能性。
