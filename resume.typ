...
- 目标：通过模拟简化版 PowerPC 604 和 620 架构，评估不同架构参数对 CPU 设计的影响。项目假设 32 位架构执行 RISC-V 指令集的子集，目标是探索和分析不同架构参数对性能的影响。
- 支持 10 条 RISC-V 指令（整型、浮点类型的算术和比较运算）。通过参数化设计，允许实验和调整不同的架构参数，如 NF（功能单元数量）、NI（指令数目）、NW（写回寄存器数量）、NR（重命名寄存器数量）和 NB（分支预测缓冲区大小），以评估这些参数对 CPU 性能的影响。
...
- 系统功能：支持多个并发事务，确保数据的一致性和隔离性。提供数据恢复，保证在系统故障时数据的原子性和持久性。
]
