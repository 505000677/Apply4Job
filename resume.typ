...
- 项目简介：
  - 该项目模拟了一种简化版的PowerPC架构，支持以下 10 条 RISC-V 指令：fld、fsd、add、addi、slt、fadd、fsub、fmul、fdiv、bne。模拟器设计旨在读取输入文件中的 RISC-V 汇编程序，识别并执行这些指令。通过参数化设计，允许实验和调整不同的架构参数，如 NF（功能单元数量）、NI（指令数目）、NW（写回寄存器数量）、NR（重命名寄存器数量）和 NB（分支预测缓冲区大小），以评估这些参数对CPU性能的影响。
- 架构设计：
